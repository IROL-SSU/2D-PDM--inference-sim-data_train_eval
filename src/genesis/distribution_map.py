## 사용 방법
## ./isaaclab.sh -p source/standalone/shelf_env/distribution_map.py --target_name can_2 --save
...
def combine_all_images(folder_path: str, obj_type: str, sigma=5):
    ...
    Parameters:
     ...
     """
    segmentation_path = os.path.join(folder_path, args_cli.target_name, obj_type,"seg")
    depth_path = os.path.join(folder_path, args_cli.target_name, obj_type,"depth")
    ...
    output_folder = os.path.join(folder_path, args_cli.target_name, obj_type, "processed_depth")
...
def create_depth_distribution_map(folder_path: str, visualization: bool, save: bool, occlusion_threshold=0.3):
    ...
    """
    scene_path = os.path.join(folder_path, args_cli.target_name, "scene","processed_depth")
    target_path = os.path.join(folder_path, args_cli.target_name, "target", "processed_depth")
    output_folder = os.path.join(folder_path, args_cli.target_name, "scene", "depth_dis_map")
    rgb_path = os.path.join(folder_path, args_cli.target_name, "scene", "rgb")
    ...
    for scene_file in scene_files:
        ...
        if not occluded_target_images:
            print(f"[WARNING] No target images with full occlusion found for {scene_file}. Skipping...")
            continue
...
def process_file(depth_file, similarity_file, output_file):
    ...
    if np.any(mask):
        ...
        # 255인 경우 target 만 저장
        combined_map = cv2.addWeighted(depth_map_norm, 0.7, output_image, 0.3, 0) # 0.2, 0.8 비율이 최종 정답 / FCN검증용 1,0, 0.0
        combined_map = (combined_map * 0.9).astype(np.uint8) # 0.8이 최종 정답 / FCN검증용 0.9
...
def process_all_maps(folder_path: str):
    ...
    """
    depth_distribution_path = os.path.join(folder_path, args_cli.target_name, "scene","depth_dis_map")
    similarity_map_path = os.path.join(folder_path, args_cli.target_name, "similarity_map")
    output_folder = os.path.join(folder_path, args_cli.target_name, "distribution_map")
    
    # 필요한 폴더들이 없으면 생성
    os.makedirs(depth_distribution_path, exist_ok=True)
    os.makedirs(similarity_map_path, exist_ok=True)
    ...
    for depth_fname, sim_fname in zip(depth_distribution_files, similarity_map_files):
        # 실제 파일 이름 사용
        depth_file = os.path.join(depth_distribution_path, depth_fname)
        similarity_file = os.path.join(similarity_map_path, sim_fname)
        
        # 출력 파일명은 depth 파일명 기반으로 생성 (.npy -> .png)
        output_fname = os.path.splitext(depth_fname)[0] + ".png"
...
if __name__ == "__main__":
    ...
    # 이미지 폴더 경로
    folder_path = "/home/haneul/2D_PDM/src/output"
    ...
    # similarity_map 폴더에 파일이 있을 경우에만 process_all_maps 실행
    similarity_map_path = os.path.join(folder_path, args_cli.target_name, "similarity_map")
    if os.path.exists(similarity_map_path) and len([f for f in os.listdir(similarity_map_path) if f.endswith(".png")]) > 0:
        process_all_maps(folder_path=folder_path)
    else:
        print(f"similarity_map 폴더에 파일이 없어서 process_all_maps를 건너뜁니다: {similarity_map_path}")